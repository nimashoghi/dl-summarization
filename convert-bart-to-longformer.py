...
def create_long_model(
    ...
    while k < max_pos - 1:
        new_encoder_pos_embed[
            k : (k + step)
        ] = model.model.encoder.embed_positions.weight[2:]
        k += step
    ...
    for i, layer in enumerate(model.model.encoder.layers):
        longformer_self_attn_for_Bart = LongformerSelfAttentionForPegasus(
            config, layer_id=i
        )

        longformer_self_attn_for_Bart.longformer_self_attn.query = (
            layer.self_attn.q_proj
        )
        longformer_self_attn_for_Bart.longformer_self_attn.key = layer.self_attn.k_proj
        longformer_self_attn_for_Bart.longformer_self_attn.value = (
            layer.self_attn.v_proj
        )

        longformer_self_attn_for_Bart.longformer_self_attn.query_global = (
            layer.self_attn.q_proj
        )
        longformer_self_attn_for_Bart.longformer_self_attn.key_global = (
            layer.self_attn.k_proj
        )
        longformer_self_attn_for_Bart.longformer_self_attn.value_global = (
            layer.self_attn.v_proj
        )
