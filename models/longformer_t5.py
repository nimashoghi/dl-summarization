    ...
    def __init__(self, config):
        ...
        if config.attention_mode == "n2":
            ...
            for i, layer in enumerate(self.encoder.block):
                layer.layer[0].SelfAttention = LongformerSelfAttentionForT5(
                    config, layer_id=i
                )
...
class LongformerSelfAttentionT5Basic(nn.Module):
    ...
    def compute_bias(self, qlen, klen):
        ...
        values = values.permute([0, 2, 1]).unsqueeze(
            0
        )  # shape (1, qlen, num_heads, klen)
        return values
    ...
    def forward(
        ...
        if attention_mask is not None:
            ...
            if max_num_extra_indices_per_batch <= 0:
                ...
                selection_padding_mask = (
                    zero_to_max_range < num_extra_indices_per_batch.unsqueeze(dim=-1)
                )
                ...
                selection_padding_mask_zeros = (selection_padding_mask == 0).nonzero(
                    as_tuple=True
                )
        else:
        ...
        if self.attention_mode == "tvm":
            ...
            attn_weights = diagonaled_mm_tvm(
                q, k, self.attention_window, self.attention_dilation, False, 0, False
            )
        ...
        if remove_from_windowed_attention_mask is not None:
            ...
            remove_from_windowed_attention_mask = (
                remove_from_windowed_attention_mask.unsqueeze(dim=-1).unsqueeze(dim=-1)
            )
            ...
            repeat_size = (
                ...
                else len(self.attention_dilation)
            )
            ...
            if self.attention_mode == "tvm":
                d_mask = diagonaled_mm_tvm(
                    ...
                    self.attention_dilation,
                    False,
                    0,
                    False,
                )
            elif self.attention_mode == "sliding_chunks":
                d_mask = sliding_chunks_matmul_qk(
                    ones, float_mask, self.attention_window, padding_value=0
                )
            elif self.attention_mode == "sliding_chunks_no_overlap":
                d_mask = sliding_chunks_no_overlap_matmul_qk(
                    ones, float_mask, self.attention_window, padding_value=0
                )
        ...
        if extra_attention_mask is not None:
            selected_k = k.new_zeros(
                bsz, max_num_extra_indices_per_batch, self.num_heads, self.head_dim
            )
        ...
        if extra_attention_mask is not None:
            ...
            attn = torch.matmul(
                ...
                selected_v.transpose(1, 2).type_as(selected_attn_probs),
            ).transpose(1, 2)
            attn_probs = attn_probs.narrow(
                -1,
                max_num_extra_indices_per_batch,
                attn_probs.size(-1) - max_num_extra_indices_per_batch,
            ).contiguous()

        if self.attention_mode == "tvm":
            v = v.float().contiguous()
            attn += diagonaled_mm_tvm(
                attn_probs,
                v,
                self.attention_window,
                self.attention_dilation,
        ...
        outputs = (
            (context_layer, attn_weights) if output_attentions else (context_layer,)
        )
        return outputs
...
class LongformerSelfAttentionForT5(nn.Module):
    def __init__(self, config, layer_id):
        ...
        self.longformer_self_attn = LongformerSelfAttentionT5Basic(
            config, layer_id=layer_id, has_relative_attention_bias=True
