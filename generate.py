...
samples = pickle.load(open("/workspaces/summarization-remote/samples-nov23.bin", "rb"))
...
# with open("data-nov23-pegasus.bin", "wb") as f:
#     pickle.dump(data, f)

data = pickle.load(open("/workspaces/summarization-remote/data-nov23.bin", "rb"))
runs_og, runs_us = data["runs_og"], data["runs_us"]
...
for r1, r2 in runs:
    ...
    for metric, value in r1["metrics"].items():
        m_us[metric] += value.recall

    for metric, value in r2["metrics"].items():
        m_them[metric] += value.recall

m_us, m_them

x = sorted(runs_og, key=lambda x: x["abstract_length"], reverse=True)[0:100]
for metric in set([*m_us.keys(), *m_them.keys()]):
    average_us = m_us[metric] / len(x)
    average_og = m_them[metric] / len(x)
